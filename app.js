...
function resetSimulation() {
    ...
    resultOverlay.classList.remove('show', 'accept', 'reject');
...
function finishSimulation(isAccepted) {
    ...
    if (isAccepted) {
        ...
        resultMessage.innerText = "ACCEPTED";
        resultOverlay.classList.remove('reject');
        resultOverlay.classList.add('show', 'accept');
    } else {
        ui.log("String REJECTED. ❌", 'error');
        ui.highlightState(currentConfig.machine.currentState, false, true);
        resultMessage.innerText = "REJECTED";
        resultOverlay.classList.remove('accept');
        resultOverlay.classList.add('show', 'reject');
    }
}
